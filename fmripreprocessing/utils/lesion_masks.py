...
def apply_transform_ants(fixed_image_T1, fixed_image_MNI, moving, transforms):
    ...
    for trans in transforms:
        transformlist.append(ants.read_transform(trans))
    
    transformed_img_T1 = ants.apply_transforms(fixed=fixed_T1, moving=moving, transformlist=transformlist[:1])
    transformed_img_MNI = ants.apply_transforms(fixed=fixed_MNI, moving=moving, transformlist=transformlist)
...
def get_files(subject_id = "sub-015", path_data = "/users2/local/alix/NF_AVC_tests", pathdata_derivatives = "/users2/local/alix/NF_AVC_derivatives_masks"):
    ...
    lesion_mask = os.path.join(path_data, subject_id, f"{subject_id}_lesion_roi.nii.gz")
...
def save_lesion_masks_derivatives(subject_id, pathdata, pathdata_derivatives):
    ...
    ants.image_write(transformed["T1w"], os.path.join(pathdata_derivatives, subject_id, "anat", f"{subject_id}_space-T1w_lesion_roi.nii.gz"))
    ants.image_write(transformed["MNI"], os.path.join(pathdata_derivatives, subject_id, "anat",f"{subject_id}_space-MNI152NLin2009cAsym_lesion_roi.nii.gz"))
    return 0